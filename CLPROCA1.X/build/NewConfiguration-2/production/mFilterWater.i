...
unsigned long flash_sec;
uint8_t Water_flow_liter_sec;
uint16_t Water_flow_liter, Water_flow_liter_temp;
uint8_t flash_DA;
uint8_t filter_liter_alarm_flag;
uint16_t filter_liter_max;

void Init_Filter_Water(uint16_t liter) {

    filter_liter_max = 5000;

    Water_flow_liter_sec = 40;

    flash_sec = 86400;

    filter_liter_alarm_flag = 0;

    Water_flow_liter = Water_flow_liter_temp = liter;
}

uint8_t get_filter_liter_alarm(void) {
    return filter_liter_alarm_flag;
}

uint16_t get_filter_liter(void) {
    return Water_flow_liter;
}

void set_filter_liter(uint16_t liter) {
    uint16_t Buf[32];
    Water_flow_liter = liter;
    FLASH_WriteWord(0x3FFE, Buf, Water_flow_liter);
}

void set_filter_liter_max(uint16_t max_liter) {
    filter_liter_max = max_liter;
}
...
void mFilter_Water_Service_Mainloop(uint8_t t_1s) {
    if (flash_DA != t_1s) {
        ...
        if (flash_sec != 0) {
            ...
            flash_sec = 86400;
            uint16_t rdata = FLASH_ReadWord(0x3FFE);
            if (rdata != Water_flow_liter) {
                Water_flow_liter_temp = Water_flow_liter;
                set_filter_liter(Water_flow_liter);
            }
        }
        if (LATDbits.LATD0 | LATDbits.LATD1 | LATCbits.LATC3) {
            if (Water_flow_liter_sec != 0) {
                Water_flow_liter_sec--;
            } else {
                Water_flow_liter_sec = 40;
                if (Water_flow_liter < filter_liter_max) {
                    Water_flow_liter++;
                }
            }
        }
        if (Water_flow_liter >= Water_flow_liter_temp + 2) {
            Water_flow_liter_temp = Water_flow_liter;
            set_filter_liter(Water_flow_liter);
        }
        if (Water_flow_liter >= filter_liter_max) {
            filter_liter_alarm_flag = 1;
        } else {
            filter_liter_alarm_flag = 0;
        }
    }
