...
# 54 "./mcc_generated_files/nco1.h" 2
...
uint16_t set_bz_timer;
uint16_t bz_off_timer, bz_on_timer;
uint8_t bz_count;
uint8_t bz_fs;
uint8_t bz_lock_fs;
uint8_t bz_task_time;

void BZ_delay_ms(uint16_t ms) {
    for (int i = 0; i < ms; i++) {
        __asm("clrwdt");
...
void re_lock_BZ_on(void) {
    ...
    _delay((unsigned long)((140)*(32000000/4000.0)));
    do { NCO1CONbits.N1EN = 0; } while(0);
    _delay((unsigned long)((230)*(32000000/4000.0)));
    do { NCO1CONbits.N1EN = 1; } while(0);
    _delay((unsigned long)((140)*(32000000/4000.0)));
    do { NCO1CONbits.N1EN = 0; } while(0);
}
...
void BZrun(uint8_t count, uint16_t timer) {
    if (!bz_lock_fs) {
        bz_off_timer = bz_on_timer = set_bz_timer = timer;
        bz_count = count;
        bz_fs = 1;
        bz_lock_fs = 1;
    }
...
void BZ_Loop(unsigned char task_time) {
    if (bz_task_time != task_time) {
        bz_task_time = task_time;
