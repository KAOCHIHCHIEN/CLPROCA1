...
unsigned char set_led_1_off_timer;
unsigned char set_led_1_on_timer;
unsigned char led_1_off_timer;
unsigned char led_1_on_timer;

unsigned char set_led_2_off_timer;
unsigned char set_led_2_on_timer;
unsigned char led_2_off_timer;
unsigned char led_2_on_timer;

unsigned char set_led_3_off_timer;
unsigned char set_led_3_on_timer;
unsigned char led_3_off_timer;
unsigned char led_3_on_timer;

unsigned char set_led_4_off_timer;
unsigned char set_led_4_on_timer;
unsigned char led_4_off_timer;
unsigned char led_4_on_timer;

unsigned char set_led_5_off_timer;
unsigned char set_led_5_on_timer;
unsigned char led_5_off_timer;
unsigned char led_5_on_timer;

unsigned char set_led_sys_off_timer;
unsigned char set_led_sys_on_timer;
unsigned char led_sys_off_timer;
unsigned char led_sys_on_timer;

unsigned char led_task_time;

void mLED_Initialize(void) {
    set_led_1_flash_time(10, 10);
    set_led_2_flash_time(10, 10);
    set_led_3_flash_time(10, 10);
    set_led_4_flash_time(10, 10);
    set_led_5_flash_time(10, 10);
    set_led_sys_flash_time(10, 10);
    do { led_sys_mode = 2; } while(0);
}

void set_led_1_flash_time(unsigned char on_time, unsigned char off_time) {
    set_led_1_off_timer = off_time;
    set_led_1_on_timer = on_time;
}

void set_led_2_flash_time(unsigned char on_time, unsigned char off_time) {
    set_led_2_off_timer = off_time;
    set_led_2_on_timer = on_time;
}

void set_led_3_flash_time(unsigned char on_time, unsigned char off_time) {
    set_led_3_off_timer = off_time;
    set_led_3_on_timer = on_time;
}

void set_led_4_flash_time(unsigned char on_time, unsigned char off_time) {
    set_led_4_off_timer = off_time;
    set_led_4_on_timer = on_time;
}

void set_led_5_flash_time(unsigned char on_time, unsigned char off_time) {
    set_led_5_off_timer = off_time;
    set_led_5_on_timer = on_time;
}

void set_led_sys_flash_time(unsigned char on_time, unsigned char off_time) {
    set_led_sys_off_timer = off_time;
    set_led_sys_on_timer = on_time;
}

void mLED_Loop(unsigned char task_time) {

    if (led_task_time != task_time) {
        led_task_time = task_time;
    ...
    if (led_1_mode == 0) {
        ...
        if (LATEbits.LATE1) {
            if (led_1_on_timer != 0) {
                ...
                LATEbits.LATE1 = 0;
                led_1_off_timer = set_led_1_off_timer;
            }
        } else {
            if (led_1_off_timer != 0) {
                led_1_off_timer--;
            } else {
                LATEbits.LATE1 = 1;
                led_1_on_timer = set_led_1_on_timer;
            }
        }
    }
    ...
    if (led_2_mode == 0) {
        ...
        if (LATEbits.LATE0) {
            if (led_2_on_timer != 0) {
                ...
                LATEbits.LATE0 = 0;
                led_2_off_timer = set_led_2_off_timer;
            }
        } else {
            if (led_2_off_timer != 0) {
                led_2_off_timer--;
            } else {
                LATEbits.LATE0 = 1;
                led_2_on_timer = set_led_2_on_timer;
            }
        }
    }
    ...
    if (led_3_mode == 0) {
        ...
        if (LATAbits.LATA5) {
            if (led_3_on_timer != 0) {
                ...
                LATAbits.LATA5 = 0;
                led_3_off_timer = set_led_3_off_timer;
            }
        } else {
            if (led_3_off_timer != 0) {
                led_3_off_timer--;
            } else {
                LATAbits.LATA5 = 1;
                led_3_on_timer = set_led_3_on_timer;
            }
        }
    }
    ...
    if (led_4_mode == 0) {
        LATBbits.LATB6 = 0;
    } else if (led_4_mode == 1) {
        LATBbits.LATB6 = 1;
    } else if (led_4_mode == 2) {
        if (LATBbits.LATB6) {
            if (led_4_on_timer != 0) {
                led_4_on_timer--;
            } else {
                LATBbits.LATB6 = 0;
                led_4_off_timer = set_led_4_off_timer;
            }
        } else {
            if (led_4_off_timer != 0) {
                led_4_off_timer--;
            } else {
                LATEbits.LATE1 = 1;
                led_4_on_timer = set_led_4_on_timer;
            }
        }
    }
    ...
    if (led_5_mode == 0) {
        LATBbits.LATB5 = 0;
    } else if (led_5_mode == 1) {
        LATBbits.LATB5 = 1;
    } else if (led_5_mode == 2) {
        if (LATBbits.LATB5) {
            if (led_5_on_timer != 0) {
                led_5_on_timer--;
            } else {
                LATBbits.LATB5 = 0;
                led_5_off_timer = set_led_5_off_timer;
            }
        } else {
            if (led_5_off_timer != 0) {
                led_5_off_timer--;
            } else {
                LATBbits.LATB5 = 1;
                led_5_on_timer = set_led_5_on_timer;
            }
        }
    }

    if (led_sys_mode == 0) {
        led_sys_flash_flag = 0;
    } else if (led_sys_mode == 1) {
        led_sys_flash_flag = 1;
    } else if (led_sys_mode == 2) {
        if (led_sys_flash_flag) {
            if (led_sys_on_timer != 0) {
                led_sys_on_timer--;
            } else {
                led_sys_flash_flag = 0;
                led_sys_off_timer = set_led_sys_off_timer;
            }
        } else {
            if (led_sys_off_timer != 0) {
                led_sys_off_timer--;
            } else {
                led_sys_flash_flag = 1;
                led_sys_on_timer = set_led_sys_on_timer;
            }
        }
    }
        }
