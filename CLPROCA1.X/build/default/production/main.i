...
# 846 "./mcc_generated_files/pin_manager.h"
void PIN_MANAGER_Initialize (void);
# 858 "./mcc_generated_files/pin_manager.h"
void PIN_MANAGER_IOC(void);
# 871 "./mcc_generated_files/pin_manager.h"
void IOCBF0_ISR(void);
# 894 "./mcc_generated_files/pin_manager.h"
void IOCBF0_SetInterruptHandler(void (* InterruptHandler)(void));
# 918 "./mcc_generated_files/pin_manager.h"
extern void (*IOCBF0_InterruptHandler)(void);
# 942 "./mcc_generated_files/pin_manager.h"
...
# 105 "./mcc_generated_files/pwm5.h"
 void PWM5_Initialize(void);
# 132 "./mcc_generated_files/pwm5.h"
...
void i2c_led_ck(void);
void set_flash_time(uint16_t time);
void BZ_RUN(uint8_t count, uint8_t timer);
# 75 "main.c"
...
byte_bit_type i2c_button, m_button;
byte_bit_type i2c_led, i2c_led_tmp;

uint8_t button_irq_flag;

byte_bit_type hot_temp_err;

uint8_t button_push_flag;
uint8_t hot_button_flag;
uint8_t warm_button_flag;
uint8_t cold_button_flag;
uint8_t All_Butter_flag;
uint8_t SET_Butter_flag;
...
uint16_t flash_led_time, flash_time;

uint8_t err_bz_flag;

uint8_t set_bz_timer;
uint8_t bz_timer;
uint8_t bz_count;
uint8_t bz_fs;
...
void main(void) {
    ...
    IOCBF0_SetInterruptHandler(I2C_Button_irq);
    ...
    for (int i = 0; i < 3; i++) {
        __asm("clrwdt");
        delay_ms(10);
        uint8_t ID = I2C1_Read1ByteRegister(1, 0x00);
        if (ID == 0x02) {
            break;
        } else {
...
# 505 "main.c"
    i2c_led.byte = 0;
    do { i2c_led.bits.p4 = 0; } while(0);
    do { i2c_led.bits.p5 = 1; } while(0);
    do { i2c_led.bits.p6 = 1; } while(0);
    do { i2c_led.bits.p7 = 1; } while(0);
    i2c_led_ck();
    do { LATBbits.LATB5 = 0; } while(0);
    BZ_on();
    delay_ms(500);
    do { i2c_led.bits.p4 = 1; } while(0);
    do { i2c_led.bits.p5 = 0; } while(0);
    do { i2c_led.bits.p7 = 0; } while(0);
    i2c_led_ck();
    do { LATBbits.LATB5 = 1; } while(0);
    BZ_on();
    delay_ms(500);
    ...
    ck_AC_soda_count = 20;
    ck_AC_soda_time = 2 * 10;

    ck_AC_chilling_count = 20;
    ck_AC_chilling_time = 2 * 10;
    ...
    ck_AC_HEATING_count = 20;
    ck_AC_HEATING_time = 2 * 10;
    ...
    eco_on_off_flag = FLASH_ReadWord(0x3FFE);
    if (!((eco_on_off_flag == 1) | (eco_on_off_flag == 0))) {
        eco_on_off_flag = 0;
        FLASH_WriteWord(0x3FFE, wdata, eco_on_off_flag);
    }

    TEMP_Data = FLASH_ReadWord(0x3FFF);
    if ((TEMP_Data == 80) | (TEMP_Data == 90) | (TEMP_Data == 100)) {
        Set_Hot_TEMP = (uint8_t) TEMP_Data;
    } else {
        Set_Hot_TEMP = 90;
    ...
    SET_Butter_Pull_Time = 5 * 10;
    Set_time = 0;
    Set_Hot_flag = 0;
    soda_water_clean_time = 2 * 10;

    hot_temp_err.byte = 0;

    for (int i = 0; i < 5; i++) {
        Hot_adc_v = 0;
        Hot_adc_v = adc_get(HOT_ADC_MCU);
        hot_start_TEMP = Hot_Temp = Thermistor(Hot_adc_v) + 3;
    ...
    m_key_sleep_time = 0;
    m_ck_level_time = 10 * 10;

    hot_start_time = 25 * 10;
    hot_start_ck_time = 60 * 10;
    ...
    i2c_button.byte = I2C1_Read1ByteRegister(1, 0x01);
    ...
    while (1) {
        ...
        if (ck_AC_Hot_flag) {
            ...
            if (Hot_adc_v > 0x3fa || Hot_adc_v < 0x02) {
                hot_temp_err.bits.p0 = 1;
            } else {
                hot_temp_err.bits.p0 = 0;
                Hot_Temp = Thermistor(Hot_adc_v) + 3;
                if (Hot_Temp >= 103) {
                    hot_temp_err.bits.p1 = 1;
                } else {
                    hot_temp_err.bits.p1 = 0;
                    if (hot_start_flag) {
                        if (ck_AC_Hot_flag) {
                            if (hot_start_time == 0) {
                                Hot_SW_ON_OFF_flag = 0;
                            ...
                            if (hot_start_ck_time == 0) {
                                if (Hot_Temp >= hot_start_TEMP + 10) {
                                    hot_temp_err.bits.p2 = 1;
                                } else {
                                    hot_temp_err.bits.p2 = 0;
                                }
                                hot_start_flag = 0;
                            }
            ...
            if (hot_temp_err.byte == 0) {
                if (ck_AC_Hot_flag) {
                    if (Hot_SW_ON_OFF_flag) {
            ...
            case 0:
                if (ck_AC_Hot_flag) {
                    if (hot_temp_err.byte == 0) {
                        if (LATFbits.LATF7) {
                            if (Reheat_flag) {
                                ...
                                if (eco_run_flag) {
                                    ...
                                    } else if (Hot_Temp > 80 && Hot_Temp < 90) {
                                        heating_led_show_count = 2;
                                    } else if (Hot_Temp > 90 && Hot_Temp < 100) {
                                        heating_led_show_count = 3;
                                    }
                                    if (heating_led_show_count == 1) {
                                        i2c_led.bits.p3 = flash_led;
                                        i2c_led.bits.p2 = 0;
                                        i2c_led.bits.p1 = 0;
                                    } else if (heating_led_show_count == 2) {
                                        i2c_led.bits.p3 = 1;
                                        i2c_led.bits.p2 = flash_led;
                                        i2c_led.bits.p1 = 0;
                            ...
                            if (eco_run_flag) {
                                ...
                                if (Hot_Temp < 85) {
                                    ...
                                    i2c_led.bits.p2 = 0;
                                    i2c_led.bits.p1 = 0;
                                } else if (Hot_Temp > 85 && Hot_Temp < 95) {
                                    i2c_led.bits.p3 = 1;
                                    i2c_led.bits.p2 = 1;
                                    i2c_led.bits.p1 = 0;
                                } else if (Hot_Temp > 95) {
                        ...
                        i2c_led.bits.p1 = i2c_led.bits.p2 = i2c_led.bits.p3 = flash_led;
                    }
                } else {
                    i2c_led.bits.p3 = 0;
                    i2c_led.bits.p2 = 0;
...
# 930 "main.c"
        ...
        if (m_key_sleep_time != 0) {
            ...
            if (hot_button_flag) {
                ...
                m_key_sleep_time = 10 * 10;
            }
            else if (warm_button_flag) {
                if (SET_Mode == 0) {
            ...
            i2c_led.bits.p6 = 0;
            i2c_led.bits.p7 = 1;
            if (hot_button_flag | cold_button_flag | warm_button_flag) {
                m_key_sleep_time = 10 * 10;
            ...
            switch (SET_Mode) {
                case 0:
                    ...
                    if (SET_Butter_Pull_Time == 0) {
                        ...
                        if (Set_Hot_TEMP == 80) {
                            SET_Mode = 1;
                        } else if (Set_Hot_TEMP == 90) {
                            SET_Mode = 2;
                        } else if (Set_Hot_TEMP == 100) {
                            SET_Mode = 3;
                        }
                        m_key_sleep_time = 10 * 10;
                        Set_time = 10 * 10;
        ...
        if (SET_Mode != 0) {
            if (Set_Hot_flag) {
                ...
                if (Set_time == 0) {
                    ...
                    i2c_led.bits.p0 = (uint8_t) eco_on_off_flag;
        ...
        if (LATCbits.LATC3 | LATDbits.LATD1 | LATDbits.LATD0) {
            ...
            Water_out_protection_time = 60 * 10;
        }
        if (hot_water_on_off_flag) {
            LATCbits.LATC3 = 1;
        } else {
        ...
        if (!SET8_flag) {
            ...
            if (m_ck_level_time != 0) {
                ...
                if (WaterLevelMode == 0) {
                    ...
                    if (!LATDbits.LATD2) {
                        protection_IN_water_timer = 300 * 10;
                        do { LATDbits.LATD2 = 1; } while(0);
                    }
                    ck_water_level_flag = 1;
                    protection_IN_water_M_timer = 30 * 10;
                ...
                if (!LATDbits.LATD2) {
                    ...
                    if (sdw_water_flag) {
                        ...
                        if (WaterSDLevelMode == 0) {
                            ...
                            if (!LATDbits.LATD3) {
                                protection_IN_SD_water_timer = 300 * 10;
                                LATDbits.LATD3 = 1;

                            }
                            ck_SD_water_level_flag = 1;
                            protection_IN_SD_water_M_timer = 30 * 10;
        ...
        if ((eco_run_flag == 1) && (Reheat_flag == 0)) {
            LATEbits.LATE0 = 0;
            LATEbits.LATE1 = 0;
            LATAbits.LATA5 = 0;
            LATAbits.LATA5 = 0;
            ...
            if (SET5_flag) {
                if (UV_CK_err_flag) {
                    LATAbits.LATA5 = flash_led;
                } else {
                    LATAbits.LATA5 = ~PORTAbits.RA3;
                }
            } else if (ck_AC_HEATING_flag) {
                LATAbits.LATA5 = 1;
            } else {
                LATAbits.LATA5 = 0;
            }
            ...
            if (UV_CK_err_flag | hot_temp_err_flag) {
                LATBbits.LATB5 = flash_led;
            } else {
                LATBbits.LATB5 = 1;
            }
        }
        ...
        if (!PORTCbits.RC1) {
            if (Full_ck_time == 0) {
                ...
                while (Full_flag) {
                    ...
                    if (full_bz_count != 0) {
                        full_bz_count--;
                        NCO1CONbits.N1EN = ~NCO1CONbits.N1EN;
                    } else {
                        NCO1CONbits.N1EN = 0;
                    }
                    LATBbits.LATB5 = ~LATBbits.LATB5;

                    if (hot_button_flag | warm_button_flag | cold_button_flag | SET_Butter_flag) {
                        full_bz_count = 10;
                        hot_button_flag = warm_button_flag = cold_button_flag = SET_Butter_flag = 0;
                    }
                    delay_ms(800);
                }
            }
...
void i2c_button_ck(void) {
    ...
    if (button_irq_flag == 1) {
        i2c_button.byte = I2C1_Read1ByteRegister(1, 0x01);
        button_irq_flag = 0;
    } else {
        ls_val = I2C1_Read1ByteRegister(1, 0x03);
        i2c_led_tmp = i2c_led;
        I2C1_Write1ByteRegister(1, 0x02, i2c_led.byte);
    }


    if (i2c_button.byte == 0xFE) {
        if (!button_push_flag) {
            button_push_flag = 1;
            hot_button_flag = 1;
        }
    ...
    else if (i2c_button.byte == 0xFB) {
        if (!button_push_flag) {
            button_push_flag = 1;
            cold_button_flag = 1;
        }
    }
    else if (i2c_button.byte == 0xF7) {
        if (!button_push_flag) {
            SET_Butter_flag = 1;
            button_push_flag = 1;
        }
    }
    else if (i2c_button.byte == 0xF8) {
        if (!button_push_flag) {
            All_Butter_flag = 1;
            button_push_flag = 1;
        }
    }
    else if (i2c_button.byte == 0xFF) {
        hot_button_flag = 0;
        cold_button_flag = 0;
        warm_button_flag = 0;
        All_Butter_flag = 0;
